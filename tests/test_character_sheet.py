...
FULL = (
    "Njiah|Everlook|PALADIN|60|PALADIN_RETRIBUTION|Vergeltung|"
...
def test_missing_sections_stay_none_instead_of_becoming_zero():
    ...
    sheet = parse_character_sheet("Bob|Everlook|MAGE|60")
...
def test_an_empty_bis_section_is_not_the_same_as_nothing_open():
    ...
    without = parse_character_sheet(
        "Bob|Everlook|MAGE|60|MAGE_FIRE|Feuer|0|0|0||0|0|1~ench|0|0|0|0|0|0~~~"
    )

    assert without["bis"] is None

    nothing_open = parse_character_sheet(
        "Bob|Everlook|MAGE|60|MAGE_FIRE|Feuer|0|0|0||0|0|1~~14|0|0|14;~~"
    )
...
def test_readiness_ignores_open_bis_slots():
    ...
    complete = parse_character_sheet(
        "Bob|Everlook|MAGE|60|MAGE_FIRE|Feuer|0|0|0||0|0|1"
...
def test_the_store_collects_one_character_per_login(store):
    ...
    store.apply(FULL)
    store.apply("Zwergi|Everlook|WARRIOR|60|WARRIOR_ARMS|Waffen|540|541|100|S|100|100|2")
...
def test_the_newest_report_comes_first(store):
    ...
    store.apply("Alt|Everlook|MAGE|60|||0|0|0||0|0|100")
    store.apply("Neu|Everlook|MAGE|60|||0|0|0||0|0|900")
...
def test_a_report_without_a_timestamp_still_sorts(store):
    ...
    store.apply("Ohne|Everlook|MAGE|60")
...
def test_a_hidden_twink_is_kept_and_still_found(store):
    ...
    store.apply("Twinki|Everlook|MAGE|60|||0|0|0||0|0|901")
...
def test_a_missing_level_counts_as_high(store):
    ...
    assert not is_high_level({"level": 59})
...
def test_the_minimum_level_follows_the_configuration(tmp_path, monkeypatch):
    ...
    store = CharacterStore(_Manager(characters_min_level=55))

    store.apply("Alt|Everlook|MAGE|55|||0|0|0||0|0|900")

    assert store.min_level() == 55
