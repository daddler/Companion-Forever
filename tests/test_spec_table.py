...
def test_all_twentyseven_specialisations_are_present():
    """
    Forever hat neun Klassen mit je drei Talentbäumen.
    """

    assert len(specs.SPECS) == 27
...
def test_the_retired_classes_are_gone():
    """
    Todesritter und Mönch gibt es in Forever nicht. Sie in der Tabelle
    stehen zu lassen wäre kein harmloser Vorrat: Lektionskatalog und
    Fähigkeitstabellen richten sich nach ihr aus.
    """

    assert specs.specs_for_class("Death Knight") == ()

    assert specs.specs_for_class("Monk") == ()


def test_the_roles_are_complete():

    assert len(specs.specs_for_role(ROLE_TANK)) == 2

    assert len(specs.specs_for_role(ROLE_HEALER)) == 5

    assert len(specs.specs_for_role(ROLE_DPS)) == 19


def test_feral_claims_no_role():
    """
    Derselbe Talentbaum trägt Katze und Bär; welche von beiden jemand
    gerade ist, steht im Kampf und nicht im Baum. "Schaden" zu
    behaupten hiesse, jeden Bären dauerhaft gegen die
    Schadensrangliste zu messen.
    """

    assert specs.role_for_spec("Druid", "Wilder Kampf") == ""

    assert specs.role_for_spec("Druid", "Feral") == ""

    assert specs.normalize_spec("Druid", "Feral") == "Wilder Kampf"
...
def test_english_names_translate_to_the_catalogue_spelling():
    ...
    assert specs.normalize_spec("Warlock", "Affliction") == "Gebrechen"

    assert specs.normalize_spec("Rogue", "Subtlety") == "Täuschung"
