...
def test_specialisation_lessons_are_preferred_over_generic_ones(demo_lessons):
    """
...
def _damage_snapshot(rows):
    ...
    return _snapshot(
        encounter=EncounterInfo(encounter_id=0, name=DEMO_ENCOUNTER),
        damage_taken=tuple(
            build_damage_taken(name, DEMO_ENCOUNTER, entries)
...
#
# "Doppelschlag" ist vermeidbar, "Ruf der Nacht" nicht.
#

_AVOIDABLE = ("Doppelschlag", 90000.0, 3, "")
_UNAVOIDABLE = ("Ruf der Nacht", 10000.0, 1, "")


def test_a_lone_player_of_their_role_is_still_rated_absolutely(demo_rules):
    """
...
def test_clean_play_still_earns_the_full_rating(demo_rules):
...
def test_survival_rates_the_share_not_the_absolute_sum(demo_rules):
    """
    ...
    snapshot = _damage_snapshot([
        ...
        # vermeidbar.
        #
        ("Panzer", (("Ruf der Nacht", 1000000.0, 40, ""),)),
        ("Spitze", (_AVOIDABLE, _UNAVOIDABLE)),
    ])
