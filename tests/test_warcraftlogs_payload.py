...
def test_full_payload_becomes_a_complete_snapshot():
    ...
    assert snapshot.encounter_name == DEMO_ENCOUNTER
...
def test_encounter_is_enriched_with_the_instance(demo_encounters):
    """
    ...
    assert snapshot.encounter.instance == demo_encounters
    assert snapshot.encounter.difficulty == "20 Heroisch"
...
def test_build_fight_list_maps_known_fields(demo_encounters):
    ...
    assert fight.encounter_name == DEMO_ENCOUNTER
    assert fight.difficulty == "20 Heroisch"
    assert fight.kill is False
    assert fight.boss_percentage == 42.5
    assert fight.pull_number == 7
    assert fight.label == f"Pull 7 · {DEMO_ENCOUNTER} · 42 % · 03:07"
...
def test_build_fight_list_drops_trash():
    ...
    fights = build_fight_list({
        "fights": [
            ...
            {"id": 2, "encounter_id": 1640, "name": DEMO_ENCOUNTER},
        ...
        players=[
            {
                ...
                "damage_taken_abilities": [
                    {"ability": "Doppelschlag", "amount": 96000.0, "hits": 2},
                    {"ability": "Ruf der Nacht", "amount": 54000.0, "hits": 3},
                ],
                "dots": [
...
def test_deep_analysis_blocks_are_mapped(demo_abilities):
...
def test_damage_taken_is_classified_using_the_fight_name(demo_rules):
    """
...
def test_cooldown_usage_counts_possible_uses_and_burst_alignment(demo_abilities):
