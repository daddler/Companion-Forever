...
def test_an_unknown_ability_is_unknown_and_not_unavoidable(demo_rules):
    """
    ...
    assert avoidable.verdict(DEMO_ENCOUNTER, "Irgendwas Neues") == (
        avoidable.VERDICT_UNKNOWN
    )

    assert avoidable.verdict("Unbekannter Boss", "Doppelschlag") == (
        avoidable.VERDICT_UNKNOWN
    )


def test_classification_never_raises_on_junk(demo_rules):
...
def test_lookup_is_case_insensitive(demo_rules):

    assert avoidable.is_avoidable(DEMO_ENCOUNTER.lower(), "DOPPELSCHLAG") is True


def test_the_encounter_table_beats_the_global_one(demo_rules):

    #
    # "Melee" ist global als unvermeidbar hinterlegt.
    #

    assert avoidable.verdict(DEMO_ENCOUNTER, "Melee") == (
        avoidable.VERDICT_UNAVOIDABLE
    )


def test_tanks_are_exempt_where_the_rule_says_so(demo_rules):
    """
...
def test_every_rule_uses_a_real_mechanic_category(demo_rules):
...
from conftest import DEMO_ENCOUNTER


def _rows():

    return (
        ("Doppelschlag", 96000.0, 2, ""),
        ("Ruf der Nacht", 54000.0, 3, ""),
        ("Völlig Unbekannt", 20000.0, 1, ""),
    )


def test_the_three_buckets_add_up_to_the_total(demo_rules):

    entry = build_damage_taken("Dolchtanz", DEMO_ENCOUNTER, _rows())
...
def test_shares_stay_within_bounds(demo_rules):
...
def test_abilities_are_sorted_by_damage(demo_rules):

    entries = classify_abilities(DEMO_ENCOUNTER, _rows())
...
def test_an_entirely_unclassified_row_is_not_usable_for_a_rating(demo_rules):
    """
...
def test_an_empty_entry_is_not_usable_either(demo_rules):
...
def test_avoidable_hits_become_mechanic_issues(demo_rules):

    entry = build_damage_taken("Dolchtanz", DEMO_ENCOUNTER, _rows())

    issues = derive_mechanics((entry,), DEMO_ENCOUNTER)
    ...
    assert "Nicht vor dem Boss stehen" in issue.mechanic


def test_unavoidable_damage_produces_no_issue(demo_rules):

    entry = build_damage_taken(
        "Bramborn",
        DEMO_ENCOUNTER,
        (("Dreifachhieb", 500000.0, 9, ""),),
        role="tank",
    )

    assert derive_mechanics((entry,), DEMO_ENCOUNTER) == ()


def test_the_bot_wins_when_both_describe_the_same_hit(demo_rules):
    """
    ...
    derived = derive_mechanics((entry,), DEMO_ENCOUNTER)
...
def test_genuinely_different_issues_both_survive(demo_rules):
    ...
    merged = merge_mechanics(bot, derive_mechanics((entry,), DEMO_ENCOUNTER))

    assert len(merged) == 2


def test_the_same_ability_for_another_player_is_not_deduplicated(demo_rules):
    """
...
def test_bot_rows_come_first(demo_rules):
    """
...
def test_every_known_boss_has_reference_data():
    """
    ...
    Die Bosslisten von Forever sind noch leer; dieser Test greift,
    sobald der erste Name darin steht. Absichtlich **ohne** das
    Demo-Fixture: gefragt ist der echte Bestand.
    """
...
def test_every_boss_has_both_verdicts_represented(demo_rules):
    """
...
def test_every_avoidable_rule_explains_what_to_do(demo_rules):
    """
...
def test_ability_labels_are_unambiguous(demo_rules):
    """
...
def test_the_alias_table_is_derived_from_the_labels(demo_rules):
    """
...
#
# Nicht jeder Kampf hat beide Urteile: ein Boss ohne klassischen
# Tankangriff hat keine unvermeidbare Zeile, und eine Fähigkeit dafür
# zu erfinden, nur damit die Tabelle gleichmäßig aussieht, wäre genau
# das Raten, das diese Referenzdaten vermeiden sollen.
#

WITHOUT_TANK_ABILITY = {"Spoils of Pandaria"}


def test_tank_abilities_are_marked_as_such_on_every_boss(demo_rules):
    """
