...
def test_live_cooldown_lists_are_derived_from_the_cast_times():
    ...
    #
    # Die Fähigkeit steht hier unter dem Namen, unter dem die Quelle
    # sie liefert. Vereinheitlicht würde sie nur, wenn
    # `analyzer/data/class_abilities.py` sie kennte - und die Tabelle
    # ist leer, solange Forever nicht erschienen ist. Genau das ist
    # der Punkt: die Wiedergabe rechnet richtig, ob eine Fähigkeit
...
def test_timeline_payload_tolerates_series_of_different_lengths():
    ...
    timeline = timeline_from_payload(
        {
            "interval": 1.0,
            "fight": {"name": DEMO_ENCOUNTER, "duration": 4.0, "raid_size": 1},
...
def test_raw_hits_are_judged_by_the_app_not_by_the_bot(demo_rules):
    """
    ...
    timeline = timeline_from_payload(
        _hits_payload([
            {"player": "Dolchtanz", "ability": "Doppelschlag",
             "at": 34.0, "amount": 96000},
            {"player": "Dolchtanz", "ability": "Ruf der Nacht",
    ...
    # "Doppelschlag" ist vermeidbar, "Ruf der Nacht" ausdrücklich nicht.
    assert abilities == ["Doppelschlag"]


def test_an_unknown_ability_produces_no_accusation(demo_rules):
    """
...
def test_unavoidable_tank_damage_is_never_counted_as_a_hit(demo_rules):
    """
    ...
    hit = {"ability": "Dreifachhieb", "at": 20.0, "amount": 300000}
