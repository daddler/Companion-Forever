...
def classify_abilities(
    ...
    `classifier` ist normalerweise `analyzer.data.avoidable.classify` -
    die Referenztabelle über das echte Spiel. Eine Quelle, deren Kampf
    gar nicht aus dem Spiel stammt, reicht stattdessen ihre eigene
    Wertung herein (`analyzer/providers/mock.py`). Das ist der
    Unterschied zwischen "die Simulation zeigt keine Befunde, weil für
    Forever noch keine Referenzdaten vorliegen" und "die Simulation
    führt vor, wie ein Befund aussieht" - und es hält Erfundenes aus
    der Referenztabelle heraus.
    """

    if classifier is None:
        classifier = avoidable_data.classify
    ...
    for ability, amount, hits, source_name in rows:
        ...
        rule = classifier(encounter_name, ability, source_name)
...
def build_damage_taken(
    ...
    `classifier` wie in `classify_abilities()`.
    """

    abilities = classify_abilities(encounter_name, rows, role, classifier)
