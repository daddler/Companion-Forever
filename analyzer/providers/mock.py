...
DER KAMPF IST ERFUNDEN, UND ER BRINGT SEINE WERTUNG SELBST MIT
--------------------------------------------------------------

Bis 5.0 bildete die Simulation einen echten Boss nach und schlug
dessen Fähigkeiten in `analyzer/data/avoidable.py` nach. Das ging
solange gut, wie dort Daten standen - für Forever stehen dort keine
(die Bosse sind nicht veröffentlicht), und die Simulation zeigte
plötzlich null vermeidbare Treffer. Nicht falsch, aber nutzlos:
gerade der Weg "Befund → Lektion → Sekunde im Pull" liesse sich dann
nicht mehr vorführen.

Deshalb trägt sie ihre Wertung jetzt selbst, in `SIMULATED_RULES`
unten. Das ist zugleich die ehrlichere Aufteilung: eine erfundene
Bossmechanik gehört nicht in eine Referenztabelle, die Auskunft über
das echte Spiel geben soll. Der Kampf heisst nach dem, was er ist,
und die Oberfläche kennzeichnet ihn als Simulation.
"""
...
#
# --------------------------------------------------
# Die Wertung dieses einen erfundenen Kampfes
# --------------------------------------------------
#
# Sie steht hier und nicht in `analyzer/data/avoidable.py`, weil sie
# nichts über das echte Spiel aussagt - siehe Modulkommentar. Aufbau
# und Bedeutung sind identisch, damit die Verdrahtung dieselbe bleibt.
#

SIMULATED_RULES: dict[str, avoidable_data.AbilityRule] = {

    rule.ability: rule
    for rule in (

        avoidable_data.AbilityRule(
            ability="Inferno",
            label="Inferno",
            category=MECHANIC_MOVEMENT,
            note="Die brennende Fläche verlassen, bevor sie zündet.",
        ),
        avoidable_data.AbilityRule(
            ability="Schwarmbefall",
            label="Schwarmbefall",
            category=MECHANIC_MOVEMENT,
            note="Vom Schwarm wegbewegen, er folgt langsam.",
        ),
        avoidable_data.AbilityRule(
            ability="Giftsalve",
            label="Giftsalve",
            category=MECHANIC_INTERRUPT,
            note="Unterbrechen - die Salve hat ein Zauberfenster.",
        ),
        avoidable_data.AbilityRule(
        ...
        avoidable_data.AbilityRule(
            ability="Zerfetzender Ansturm",
            label="Zerfetzender Ansturm",
            category=MECHANIC_MOVEMENT,
            note="Der Ansturm wird angekündigt - aus der Bahn gehen.",
        ),

        #
        # Die beiden unvermeidbaren: sie stehen hier, damit die
        # Simulation auch den Fall "eingeordnet, aber kein Vorwurf"
        # zeigt - ohne ihn sähe jede Einordnung wie ein Befund aus.
        #
...
def _simulated_classifier(encounter_name, ability, source_name=""):
    """
    Die Form, die `analyzer/analysis/damage.py` erwartet. Der
    Kampfname spielt hier keine Rolle - es gibt genau einen.
    """

    return simulated_rule(ability)


def simulated_rule(ability: str):
    """
    Die Wertung einer Fähigkeit dieses Kampfes.

    Erst die eigene Tabelle, dann die kampfunabhängigen Wahrheiten aus
    `analyzer/data/avoidable.py` (Sturzschaden, Nahkampfangriff) - die
    gelten auch für einen erfundenen Boss.
    """

    rule = SIMULATED_RULES.get(ability)

    if rule is not None:
        return rule

    return avoidable_data.classify("", ability)
...
class MockRaidDataProvider(RaidDataProvider):
    ...
    #
    # Erfunden und als solcher benannt. Ein echter Bossname hier wäre
    # eine Behauptung über einen Kampf, den noch niemand gesehen hat.
    #
    ...
    def _combat_snapshot(
        ...
        mechanics = damage_analysis.merge_mechanics(
            ...
            damage_analysis.derive_mechanics(
                damage_taken,
                self._encounter_name,
                _simulated_classifier,
            ),
        )
        ...
        for actor, _base in _ACTORS:
            ...
            entries.append(
                damage_analysis.build_damage_taken(
                    ...
                    rows=tuple(rows),
                    role=actor.role,
                    classifier=_simulated_classifier,
                )
            )
